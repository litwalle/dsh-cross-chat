...
function buildRelayMessage(sender, text, attribution, senderTitle) {
	...
	return createUserMessage({
		content: [{
			type: "text",
			text: attribution === "prefix" ? `[跨会话消息 · 来自 ${label} (${sender.id})]\n\n${text}` : text
		}],
		source: {
...
async function deliverFiles(targetCwd, paths, kind, readErrorPrefix) {
	...
	for (const p of paths) {
		...
		await writeFile(await dedupeTargetPath(targetCwd, base), data);
...
function createSessionTool(ctx, config) {
	return defineTool({
		...
		description: "新建一个顶层对话（与 GUI 新建对话相同）。可选 cwd（默认用你所在会话的工作目录）、title（标题）、first_message（开场消息：发送后新会话立即开始工作并出现在侧边栏；不传则创建空白会话，直到收到第一条消息才可见）。若 cwd 属于某个已注册工作区，新会话会挂入该工作区分组（否则落入「未分组」）。返回的 session_id 可用于 send_session_message / read_session 与其通信；新会话的 agent 也有同样的工具，可以回你消息。",
		parameters: {
			cwd: {
				type: "string",
		...
		async execute(args, exec) {
			...
			*/
			const workspaceIdFor = async (directory) => {
				const canonical = await canonicalDirectory(directory);
				try {
					return unwrap((await api.workspace.list({
						rpcId: RpcId(randomUUID()),
						payload: {}
					})).result, "WORKSPACE_LIST_FAILED", "").items.find((workspace) => workspace.path === canonical)?.workspaceId;
				} catch {
					return;
				}
			};
			...
			if (cwd !== void 0) {
				...
				if (ownerId !== void 0) payload.workspaceId = ownerId;
